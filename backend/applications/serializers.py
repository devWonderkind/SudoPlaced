...
class KeyNoteSerializer(serializers.ModelSerializer):
    ...
    def get_application_details(self, obj):
        if obj.application:
            return {
                ...
                "company_name": obj.application.company_name,
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user and user.is_authenticated:
            self.fields["application_id"].queryset = JobApplication.objects.filter(
                user=user
            )

    def create(self, validated_data):
        return KeyNote.objects.create(**validated_data)
...
class JobApplicationDetailSerializer(serializers.ModelSerializer):
    ...
    def validate_status(self, value):
        ...
        if value and value.user != user:
            raise serializers.ValidationError(
                "Invalid status. You can only assign statuses that belong to your account."
            )
        return value
    ...
    def validate_hr_contact_ids(self, value):
        ...
        valid_contacts = (
            ProfessionalContact.objects.filter(id__in=contact_ids)
            .filter(
    ...
    def create(self, validated_data):
        ...
        if "status" not in validated_data or validated_data["status"] is None:
            ...
            default_status = ApplicationStatus.objects.filter(
                user=user, name="Bookmarked"
            ).first()
            if not default_status:
                default_status = (
                    ApplicationStatus.objects.filter(user=user)
                    .order_by("order")
                    .first()
                )
